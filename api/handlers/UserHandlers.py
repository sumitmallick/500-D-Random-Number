...
class GenerateRandomNumber(Resource):
    @auth.login_required
    def get(self):
        def generate_random_array(dimension=500, min_value=-0, max_value=100):
            return jsonify({"data": [random.uniform(min_value, max_value) for _ in range(dimension)]})
        return generate_random_array(500, 0, 100)